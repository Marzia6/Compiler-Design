...
%token IF ELSE FOR WHILE DO BREAK CONTINUE RETURN INT FLOAT CHAR VOID DOUBLE SWITCH CASE DEFAULT PRINTLN COLON GOTO ADDOP MULOP INCOP DECOP RELOP ASSIGNOP LOGICOP NOT LPAREN RPAREN LCURL RCURL LTHIRD RTHIRD SEMICOLON COMMA ID CONST_INT CONST_FLOAT 
%nonassoc IF
%nonassoc ELSE
...
argument_list : arguments
	...
	| /* epsilon */
	{
		...
		$$ = new symbol_info("epsilon","argument_list");
	}
	;
...
int main(int argc, char *argv[])
{
	...
	yyin = fopen(argv[1], "r");
	outlog.open("21201571_log.txt", ios::trunc);
	errorlog.open("21201571_error.txt", ios::trunc);
	st.enter_scope();
